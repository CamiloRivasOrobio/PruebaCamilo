...
namespace sistema.Controllers
{
    public class CitaController : Controller
    {
        ...
        public CitaController(ICitas citasRepository, IPacientes pacientesRepository, IEmpleado empleadoRepository, ICuentasPorCobrar cuentaPorCobrarRepository,
        ...
        public async Task<IActionResult> AgendarCita(CitaAddViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction("CalendarioLineal", new { buscar = viewModel.HoraYFecha.ToString("yyyy/MM/dd") });

            }
            return View(viewModel);
        }

        public async Task<IActionResult> BloquearFecha(string fecha)
        {
            var user = await _userManager.GetUserAsync(HttpContext.User);
        ...
        public async Task<IActionResult> CancelarCita(CitasCancelViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var cuentaPorCobrarPendiente =
                        _cuentaPorCobrarRepository.GetUltimaCuentaPendientePaciente((int)viewModel.Cita.PacienteId);
                    cuentaPorCobrarPendiente.Valor += viewModel.CargoPenalizacion;
                    cuentaPorCobrarPendiente.Observaciones += $". Se agrega cargo por penalización de cita cancelada el día" +
                        $"{DateTime.Now}";
                    _cuentaPorCobrarRepository.Update(cuentaPorCobrarPendiente);
                }

                TempData["Message"] = "¡La cita se ha cancelado con exito.!";
