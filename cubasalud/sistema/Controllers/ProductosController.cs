...
namespace sistema.Controllers
{
    ...
    public class ProductosController : Controller
    {
        ...
        public string ConsultarUnidadesCompra()
        {
            try
            {
                var unidades = _productosRepository.GetUnidadesCompra();
                return JsonSerializer.Serialize(new { Exitoso = true, Resultado = unidades });
            }
            catch (Exception ex)
            {
                ...
                {
                    Exitoso = false,
                    Mensaje = "Error al consultar unidades de origen. " + ex.Message
                });
            }
        }
        [HttpPost]
        public string ConsultarUnidadesVenta()
        {
            try
            {
                var unidades = _productosRepository.GetUnidadesVenta();
                return JsonSerializer.Serialize(new { Exitoso = true, Resultado = unidades });
            }
            catch (Exception ex)
            {
        ...
        #region Consulta Categorias, marcas, grupos y presentaciones
        [HttpPost]
        ...
        #region Clinica insumos medicos
        [Authorize(Roles = "Administrador, Supervisor, Vendedor")]
        public IActionResult ClinicaInsumosMedicos(int? categoriaId)
        {
            var viewModel = new InsumosFarmaciaViewModel();

            viewModel.currentFilter = viewModel.buscar;
            viewModel.Init(_categoryRepository);
            viewModel.TotalInsumos = _productosRepository.GetTotalInsumosClinica(categoriaId);
            viewModel.ProductosInventario = _productosRepository.GetInventarioProductos((int)TipoBodegaEnum.Clinica,
                (int)TipoProductoEnum.InsumosMedicos, categoriaId);

            return View(viewModel);
        }
        [Authorize(Roles = "Administrador, Supervisor")]
        public IActionResult ClinicaInsumosMedicosNuevo()
        {
            var modelo = new ProductoInsumoMedicoViewModel();
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        [HttpPost]
        public string ClinicaInsumosMedicosNuevo(ProductoInsumoMedicoViewModel model)
        {
            #region Validaciones
            if (model.Nombre == null || model.Nombre.Trim() == "")
            {
                return JsonSerializer.Serialize(new { Exitoso = false, Mensaje = "Proporcione el nombre del producto" });
            }
            #endregion
            try
            {
                ...
                {
                    TipoProductoId = (int)TipoProductoEnum.InsumosMedicos,
                    TipoBodegaId = (int)TipoBodegaEnum.Clinica,
                ...
                TempData["Message"] = "¡El insumo se ha guardado con éxito.!";
            ...
            catch (Exception ex)
            {
                ...
                {
                    Exitoso = false,
                    Mensaje = "Error al registrar insumo médico. " + ex.Message
                });
            }
        }
        [HttpPost]
        public string ConsultarEquivalencias(int? productoId)
        {
            try
            {
                var listaEquivalencias = new List<EquivalenciaProductoInsumoMedicoViewModel>();
                ...
                if (equivalenciasProducto != null || equivalenciasProducto.Count > 0)
                {
                    ...
                    {
                        var equivalenciaProducto = new EquivalenciaProductoInsumoMedicoViewModel
                        {
                            Id = equivalencia.Id,
                ...
                return JsonSerializer.Serialize(new { Exitoso = true, Resultado = listaEquivalencias });
            }
            catch (Exception ex)
            {
                return JsonSerializer.Serialize(new { Exitoso = false, Mensaje = "Error al consultar equivalencias. " + ex.Message });
            }
        }
        public IActionResult ClinicaInsumosMedicosModificar(int? id)
        {
            if (id == null)
            {
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        [HttpPost]
        public string ClinicaInsumosMedicosModificar(ProductoInsumoMedicoViewModel model)
        {
            #region Validaciones
            if (model.Nombre == null || model.Nombre.Trim() == "")
            {
            ...
            try
            {
                ...
                producto.GrupoId = model.GrupoId;
                ...
                if (model.Equivalencias != null && model.Equivalencias.Count > 0)
                {
                    if (producto.ProductoEquivalencias != null && producto.ProductoEquivalencias.Count > 0)
                    {
                        ...
                        {
                            var existente = model.Equivalencias.Where(p => p.Id == equivalenciaBd.Id).FirstOrDefault();
                            if (existente == null)
                            {
                ...
                else
                {
                    if (producto.ProductoEquivalencias != null && producto.ProductoEquivalencias.Count > 0)
                    {
                        foreach (var item in producto.ProductoEquivalencias)
                        {
                            item.Eliminada = true;
                        }
                    }
                ...
                TempData["Message"] = "¡El insumo se ha modificado con éxito.!";
            ...
            catch (Exception ex)
            {
                ...
                {
                    Exitoso = false,
                    Mensaje = "Error al modificar insumo médico. " + ex.Message
                });
            }
        }
        [Authorize(Roles = "Administrador, Supervisor")]
        public IActionResult EliminarInsumosClinica(int? id)
        {
            ...
            return RedirectToAction("ClinicaInsumosMedicos");
        }
        #endregion
        ...
        [Authorize(Roles = "Administrador, Supervisor, Vendedor")]
        public IActionResult MedicamentosLaboratorio(MedicamentosFarmaciaViewModel viewModel)
        {
            if (viewModel.buscar != null)
            {
                viewModel.pageNumber = 1;
            ...
            viewModel.currentFilter = viewModel.buscar;
            viewModel.Init(_categoryRepository);
            viewModel.Productos = _productosRepository.PaginacionProductosLabMedicamentos(viewModel.buscar, viewModel.pageNumber, 25, viewModel.terapeuticoId);
            viewModel.TotalMedicamentos = _productosRepository.GetTotalMedicamentosLab();

            return View(viewModel);
        }

        [Authorize(Roles = "Administrador, Supervisor, Vendedor")]
        public IActionResult InsumosMedicosLaboratorio(InsumosFarmaciaViewModel viewModel)
        {
            if (viewModel.buscar != null)
            {
                viewModel.pageNumber = 1;
            ...
            viewModel.currentFilter = viewModel.buscar;
            viewModel.Init(_categoryRepository);
            viewModel.TotalInsumos = _productosRepository.GetTotalInsumosLab();
            viewModel.Productos = _productosRepository.PaginacionProductosLabInsumosMedicos(viewModel.buscar, viewModel.pageNumber, 25, viewModel.categoriaId);
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        public IActionResult NuevoLaboratorioMedicamentos()
        {
            // var cargarCategorias = _categoryRepository.ListarCategorias();
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        [HttpPost]
        public IActionResult NuevoLaboratorioMedicamentos(ProductoBaseViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    model.Producto.TipoBodegaId = 4;
                    model.Producto.TipoProductoId = 10; // Medicamentos
                    _productosRepository.Add(model.Producto);
                    TempData["Message"] = "¡El producto se ha guardado con éxito.!";
                    return RedirectToAction("MedicamentosLaboratorio");
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        public IActionResult NuevoLaboratorioInsumosMedicos()
        {
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        [HttpPost]
        public IActionResult NuevoLaboratorioInsumosMedicos(ProductoInsumosBaseViewModel model)
        {
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        public IActionResult ModificarMedicamentosLaboratorio(int? id)
        {
            if (id == null)
            {
                return BadRequest("request is incorrect");
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        [HttpPost]
        public IActionResult ModificarMedicamentosLaboratorio(ProductoBaseViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    TempData["Message"] = "¡El producto se ha modificado con éxito.!";
                    return RedirectToAction("ModificarMedicamentosLaboratorio", new { id = model.Producto.Id });
                }
                catch (System.Exception)
                {
                    TempData["Message"] = "ERROR, ESTE PRODUCTO YA EXISTE EN ESTA FARMACIA. ";
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        public IActionResult ModificarInsumosLaboratorio(int? id)
        {
            if (id == null)
            {
                return BadRequest("request is incorrect");
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        [HttpPost]
        public IActionResult ModificarInsumosLaboratorio(ProductoInsumosBaseViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    TempData["Message"] = "¡El producto se ha modificado con éxito.!";
                    return RedirectToAction("ModificarInsumosLaboratorio", new { id = model.Producto.Id });
                }
                catch (System.Exception)
                {
                    TempData["Message"] = "ERROR, ESTE PRODUCTO YA EXISTE EN ESTA FARMACIA. ";
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        public IActionResult EliminarMedicamentoLaboratorio(int? id)
        {
            if (id == null)
            {
                return BadRequest("request is incorrect");
        ...
        [Authorize(Roles = "Administrador, Supervisor")]
        public IActionResult EliminarInsumoLaboratorio(int? id)
        {
            if (id == null)
            {
                return BadRequest("request is incorrect");
            ...
            TempData["Message"] = "¡El producto se ha eliminado con éxito.!";
            return RedirectToAction("InsumosMedicosLaboratorio");
        }
