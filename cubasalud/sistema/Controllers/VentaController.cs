...
namespace sistema.Controllers
{
    public class VentaController : Controller
    {
        ...
        ICajaClinica cajaClinicaRepository, UserManager<User> userManager, IPacientes pacienteRepository, IConsultas consultasRepository)
        {
        ...
        [Authorize(Roles = "Administrador, Vendedor, Supervisor")]
        public async Task<IActionResult> NuevaVentaClinica(int? idConsulta)
        {
            var user = await _userManager.GetUserAsync(HttpContext.User);

            var empleadoId = user.EmpleadoId;

            var modelo = new VentaClinicaBaseViewModel();
            modelo.Init(_productoRepository, _ventaRepository, _pacienteRepository, _envioRepository);
            modelo.EmpleadoId = (int)empleadoId;

            var listausers = _ventaRepository.GetUsersRole("Mensajero");

            modelo.ListaUsuarios = listausers;

            ViewBag.ConsultaId = 0;

            if (idConsulta != null)
            {
                var consulta = _consultasRepository.GetConsulta(Convert.ToInt32(idConsulta));
                if (consulta.ConsultasServicios != null && consulta.ConsultasServicios.Count > 0)
                {
                    var servicios = "";
                    ...
                    ViewBag.ServiciosConsulta = servicios;
                    ViewBag.ConsultaId = idConsulta;
                }
            }
        ...
        [Authorize(Roles = "Administrador, Vendedor, Supervisor")]
        public ActionResult NuevaVentaUnificada()
        {
            return View();
        }
